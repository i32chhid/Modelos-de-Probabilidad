...
function throwAdice(button) {
   ...
   if (button != "restart" && validateAdice(button, diceType) === true) {
       ...
       switch(button) {
           ...
           // Botón "Uno a uno"
           case "oneByone":     
               showDoughnutChart(calcAdiceProb(parseInt(1,10), button, diceType));
               break;
           ...
           case "fixedNum":
               ...
               showDoughnutChart(calcAdiceProb(parseInt(fixedThrows,10), button, diceType));
               break;
    ...
    if (button != "restart") {
        ...
        if (diceType === "") {
            ...
            document.querySelectorAll(".diceProb input").forEach((el) => {
                ...
                if (el.value === parseFloat(0).toFixed(1) && el.id != "pX6" || el.value < 0 || el.value >= 1) {
                    validateProb = false;
...
function calcAdiceProb(throws, button, diceType) {
    ...
    for (let i=0; i<throws; i++) {
        ...
        simulations[6] = parseInt(simulations[6],10) + 1;
    }
...
function showDoughnutChart(probabilities) {
    ...
    let legend = [];
    for (i=1; i<7; i++) {
        legend.push(aDiceLegends[i]);
    }

    // Actualización del gráfico de donut
    myChart.data.labels = legend;
    myChart.options.title.text = aDiceLegends[0];
    ...
    document.querySelector(".graphic").style.minHeight = "150px";
...
function showAdiceLegend() {
   ...
   document.querySelectorAll(".bottomLegend span").forEach((el) => {
    ...
    document.getElementById("label1").innerHTML = aDiceLegends[7]; 
    document.getElementById("text1").innerHTML = parseInt(simulations[6],10);
    document.querySelector(".divText2").style.display = "block";
    document.getElementById("label2").innerHTML = aDiceLegends[8];
