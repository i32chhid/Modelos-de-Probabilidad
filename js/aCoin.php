...
function throwAcoin(button) {
    ...
    if (button != "restart" && validatePx() == true) {
        ...
        switch(button) {
            ...
            // Botón "Uno a uno"
            case "oneByone": 
                showLineGraph(calcCoinsProb(parseInt(1), parseInt(1), button));
                break;
            ...
            case "fixedNum":
                ...
                showLineGraph(calcCoinsProb(parseInt(fixedThrows), parseInt(1), button));
                break;
...
?>
    ...
    if (myChart.data.labels.length == 0) {
        ...
        xLabels[0] = parseInt(myChart.data.labels.length-1)+2;
    }
    ...
    myChart.data.datasets[0].label = legend[0];
    myChart.data.datasets[1].label = legend[1];
    myChart.config.options.scales.xAxes[0].scaleLabel.labelString = legend[2];
    myChart.config.options.scales.yAxes[0].scaleLabel.labelString = legend[3];
    myChart.update();  
...
function showAcoinLegend() {
    ...
    document.getElementById("text2").innerHTML = parseInt(simulations[1]);
    document.querySelector(".divText3").style.display = "block";
    document.getElementById("label3").innerHTML = legend[langActive][2];
    ...
    document.getElementById("label4").innerHTML = legend[langActive][3];
    document.getElementById("text4").innerHTML = parseInt(simulations[0]);  
    document.querySelector(".divText5").style.display = "block";  
    document.getElementById("label5").innerHTML = legend[langActive][4];  
